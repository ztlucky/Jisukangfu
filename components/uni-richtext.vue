<template>
	<view class="contentview">
 		<view class="mask" v-show="showMask" @click="hideInputPopup"></view>
  		 <view class="popup popupeditView"  style="height:500px;  " v-show="showInsertTextPopup">
			<!-- <view class="toolbar">
			...
			<view class="input-content">
 				<textarea class="inputArea"  maxlength="-1" v-model="textareaData" :style="textareaDataStyle"  ></textarea>
			</view>
			<view  class="popopviewbottom">
		...
		<scroll-view scroll-y="true" class="content">
			...
			<view ref="richtext" v-for="(item,index) in richList" :key="index" :style="index==richListIndex?'background:#cce0f2;':''" @click="showInputPopup(index,item)">
				...
				<image class="bannerimage" mode="widthFix" v-if="item.tagType=='image'" :src="item.value"></image>
				<video v-if="item.tagType=='video'" class="bannerimage" :src="item.value" controls></video>
			</view>
		</scroll-view>
...
.contentview{
	...
	.popup {
		...
		-webkit-box-shadow: 0upx 0upx 30upx rgba(0, 0, 0, .1);
		box-shadow: 0 0 850upx rgba(0, 0, 0, .1);
 	}
.popupeditView{
	display: flex;
	width: 92%;
