<template>
	<view class="view">
		<nav-bar bgColor="#31D880" backState="1000" fontColor="#ffffff" title="疾病诊断" type="ordinary">
			<view slot="right" v-if="isEdit" @click="save" class="navRight">保存</view>
		</nav-bar>
		<view class="list" v-if="list.length!=0">
			<view class="item" @click="toPage(isEdit?k:'/pages/work/symptoms/symptoms?id='+v.id,!isEdit)" v-for="(v,k) in list"
			 :key="k">
				<view class="itemLeft"></view>
				<view class="itemRight">
					<view class="rightText hidden">{{v.name}}</view>
					<view class="rightView" v-if="!isEdit">
						<view>{{v.symptomList.length}}</view>
		...
		<view class="bottomNav" v-else>
			...
			<view class="" @click="toPage('/pages/work/symptoms/symptoms')">添加新的分类</view>
		</view>
	</view>
</template>
