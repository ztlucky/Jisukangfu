<template>
	<view class="view">
		...
		<view class="list" v-if="list.length !=0">
			<view class="item" @click="toPage(isEdit?k:('/pages/work/problemInfo/problemInfo?id=' + v.id),!isEdit,k)" v-for="(v,k) in list" :key="k">
				<view class="itemLeft"></view>
				<view class="itemRight">
					<view class="rightText hidden">{{v.title}}</view>
					<view class="rightView" v-if="!isEdit">
						<view>{{v.type == 1?'单选题':v.type == 2?'多选题':'说明题'}}</view>
